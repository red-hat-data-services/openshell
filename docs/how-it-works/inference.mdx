...
```shell
openshell sandbox provider attach inference-demo nvidia-prod --wait
openshell sandbox provider list inference-demo
```

Running sandboxes poll for provider and effective-policy changes. `--wait`
confirms the sandbox has applied the attachment. Launch a new process so it
receives the new credential placeholder:

```shell
openshell sandbox exec --name inference-demo -- python app.py
```

An already-running process does not gain new environment variables. Credential
rotation and detach take effect once the sandbox applies the provider change.
Use `--wait` to confirm that change before relying on it.

Detach the provider to revoke its policy and credential access:

```shell
openshell sandbox provider detach inference-demo nvidia-prod --wait
```
...
openshell provider create \
  ...
  --type nvidia-native \
  --from-existing
openshell sandbox provider attach inference-demo nvidia-native-prod --wait
```
