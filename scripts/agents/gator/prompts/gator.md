...
- At the start of every watch cycle, read `payload_version` from
  ...
  `OPENSHELL_AGENT_RESULT {"status":"terminal_failure","reason":"stale_gator_payload","notes":"This sandbox is using an outdated Gator payload. Relaunch it before monitoring continues."}` so the operator relaunches the immutable watcher.
...
- Every result sentinel must include `notes` containing one or two concise, plain-language sentences that a human can read aloud. Summarize the current diagnosis, name any notable issue or unanswered question, and state the next useful action or owner. Do not repeat mechanical metadata, dump raw logs, or include secrets; mention a SHA only when it is essential to the diagnosis.
- In `watch` runtime mode, when the next action is to keep waiting, use this exact final-line format with a reason, poll interval, and curated notes: `OPENSHELL_AGENT_RESULT {"status":"waiting","next_poll_seconds":{{POLL_INTERVAL_SECONDS}},"reason":"checks_pending","notes":"Required checks are still running on the current head. Gator will inspect the results next cycle."}`. Use `blocked` when waiting on a human/process blocker, `complete` when the issue or PR reached a terminal state, `terminal_failure` for unrecoverable errors, and `transient_failure` only when the supervisor should retry soon.
- If required GitHub REST reads or writes fail with `EOF`, `Empty reply from server`, or sandbox `NET:FAIL` after policy allowed the endpoint, stop the cycle with `OPENSHELL_AGENT_RESULT {"status":"transient_failure","next_poll_seconds":120,"reason":"github_transport_eof","notes":"GitHub transport failed after policy allowed the request. The supervisor should retry without changing the PR state."}` rather than marking the issue or PR blocked.
- If the `principal-engineer-reviewer` sub-agent fails before producing usable review output, stop the cycle with `OPENSHELL_AGENT_RESULT {"status":"transient_failure","next_poll_seconds":120,"reason":"reviewer_subagent_failed","notes":"The reviewer did not produce usable feedback, so no PR state was changed. The supervisor should retry the review."}`. Do not post a marked gator comment or PR review, do not apply `gator:blocked`, and do not consume the one-disposition-per-head-SHA slot for sub-agent auth, token-refresh, transport, command, empty-output, malformed-output, or sub-agent-only policy failures.
- In `once` runtime mode, run one bounded cycle unless the operator explicitly asks you to watch inline. Still print `OPENSHELL_AGENT_RESULT {"status":"complete","reason":"one_shot_complete","notes":"The requested one-shot reconciliation is complete."}` when finished.
